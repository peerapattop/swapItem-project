...
class _Offer_comeState extends State<Offer_come> {
  ...
  void _loadPostData1(String postUid) {
    ...
    FirebaseDatabase.instance
        ...
        .equalTo(postUid)
        .once()
        .then((DatabaseEvent databaseEvent1) {
      if (databaseEvent1.snapshot.exists) {
        ...
        databaseEvent1.snapshot.children.forEach((offerSnapshot) {
          ...
          if (key != null && offerSnapshot.value != null) {
            offers[key] = Map<String, dynamic>.from(offerSnapshot.value as Map);
          }
        });

        setState(() {
          postData1 = offers; // This will update your UI with the new offers
        });

        // Print the offers map for debugging
        print('Offers loaded from Firebase:');
      ...
      // Handle errors here
      print('An error occurred while loading offers data: $error');
    });
  }
  ...
  Widget buildCircularNumberButton(int index, Map<dynamic, dynamic> postData) {
    return InkWell(
      onTap: () {
        setState(() {
          _loadPostData1(selectedPost!['post_uid'].toString());
